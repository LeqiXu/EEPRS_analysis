...
disease="AD"
AD_icd10 <- c(paste0('G30',c(0,1,8,9)),
	      paste0('F00',c(0:2,9)))
AD_icd9 <- NULL
AD_ops <- NULL
AD_s1 <- c(1289)
...
ukbb_AD = case_summary[,c("eid","AD")]
ukbb_AD = na.omit(ukbb_AD)
ukbb_AD$AD = ifelse(ukbb_AD$AD == 0, 0, 1)
print(table(ukbb_AD$AD))

write.table(ukbb_AD,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)