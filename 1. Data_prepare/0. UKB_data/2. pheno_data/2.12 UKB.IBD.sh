...
disease='IBD'
IBD_icd10 <- c(paste0('K50',c(0,1,8,9)),
...
ukbb_IBD = case_summary[,c("eid","IBD")]
ukbb_IBD = na.omit(ukbb_IBD)
ukbb_IBD$IBD = ifelse(ukbb_IBD$IBD == 0, 0, 1)
print(table(ukbb_IBD$IBD))

write.table(ukbb_IBD,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)