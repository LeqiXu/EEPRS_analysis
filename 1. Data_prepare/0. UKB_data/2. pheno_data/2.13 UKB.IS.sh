...
disease="IS"
IS_icd10 <- c(paste0('I60',0:9),
                   paste0('I61',c(0:6,8,9)),
                   paste0('I62',c(0,1,9)),
                   paste0('I63',c(0:6,8,9)),
                   'I64')
IS_icd9 <- c('4309','4319','4349','4369')
IS_ops <- NULL
IS_s1 <- c(1075)
IS_s2 <- NULL
...
ukbb_IS = case_summary[,c("eid","IS")]
ukbb_IS = na.omit(ukbb_IS)
ukbb_IS$IS = ifelse(ukbb_IS$IS == 0, 0, 1)
print(table(ukbb_IS$IS))

write.table(ukbb_IS,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)