...
disease="LuC"
LuC_icd10 <- c((paste0('C34',0:9)))
LuC_icd9 <- c(paste0('162',c(3,9)))
LuC_ops <- NULL
LuC_s1 <- c(1001,1027,1028) # field 20001
LuC_s2 <- NULL
...
selfreport_20001 <- fread('/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/ukbb_pheno_data/self_report_20001.tsv')
...
field_selfreport1 <- c("f.eid",colnames(selfreport_20001)[!is.na(str_extract(colnames(selfreport_20001),'20001'))])
...
selfreport1 <- selfreport_20001[,..field_selfreport1]
...
ukbb_LuC = case_summary[,c("eid","LuC")]
ukbb_LuC = na.omit(ukbb_LuC)
ukbb_LuC$LuC = ifelse(ukbb_LuC$LuC == 0, 0, 1)
print(table(ukbb_LuC$LuC))

write.table(ukbb_LuC,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)