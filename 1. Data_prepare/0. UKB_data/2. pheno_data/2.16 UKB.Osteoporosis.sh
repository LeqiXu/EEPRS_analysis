...
Osteoporosis_icd10 <- c(paste0('M800',c(0,1,3,5,7:9)),
                   ...
                   paste0('M819',0:9))
Osteoporosis_icd9 <- NULL
Osteoporosis_ops <- NULL
Osteoporosis_s1 <- c(1309)
Osteoporosis_s2 <- NULL
...
ukbb_Osteoporosis = case_summary[,c("eid","Osteoporosis")]
ukbb_Osteoporosis = na.omit(ukbb_Osteoporosis)
ukbb_Osteoporosis$Osteoporosis = ifelse(ukbb_Osteoporosis$Osteoporosis == 0, 0, 1)
print(table(ukbb_Osteoporosis$Osteoporosis))

write.table(ukbb_Osteoporosis,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)