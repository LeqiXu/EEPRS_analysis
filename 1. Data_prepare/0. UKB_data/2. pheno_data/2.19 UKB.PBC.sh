...
ukbb_PBC = case_summary[,c("eid","PBC")]
ukbb_PBC = na.omit(ukbb_PBC)
ukbb_PBC$PBC = ifelse(ukbb_PBC$PBC == 0, 0, 1)
print(table(ukbb_PBC$PBC))

write.table(ukbb_PBC,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)