...
disease="Angina"
Angina_icd10 <- c(paste0('I20',c(0,1,8,9)))
Angina_icd9 <- c('4139')
Angina_ops <- NULL
Angina_s1 <- c(1074)
Angina_s2 <- NULL
...
ukbb_Angina = case_summary[,c("eid","Angina")]
ukbb_Angina = na.omit(ukbb_Angina)
ukbb_Angina$Angina = ifelse(ukbb_Angina$Angina == 0, 0, 1)
print(table(ukbb_Angina$Angina))

write.table(ukbb_Angina,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)