...
ukbb_PSC = case_summary[,c("eid","PSC")]
ukbb_PSC = na.omit(ukbb_PSC)
ukbb_PSC$PSC = ifelse(ukbb_PSC$PSC == 0, 0, 1)
print(table(ukbb_PSC$PSC))

write.table(ukbb_PSC,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)