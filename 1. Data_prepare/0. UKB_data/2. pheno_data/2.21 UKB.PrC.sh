...
disease="PrC"
PrC_icd10 <- c('C61')
PrC_icd9 <- c(paste0('185',c(9)))
PrC_ops <- NULL
PrC_s1 <- c(1044) # field 20001
PrC_s2 <- NULL
...
ukbb_PrC = case_summary[,c("eid","PrC")]
ukbb_PrC = na.omit(ukbb_PrC)
ukbb_PrC$PrC = ifelse(ukbb_PrC$PrC == 0, 0, 1)
print(table(ukbb_PrC$PrC))

write.table(ukbb_PrC,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)