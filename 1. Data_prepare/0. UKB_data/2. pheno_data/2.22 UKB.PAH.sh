...
ukbb_PAH = case_summary[,c("eid","PAH")]
ukbb_PAH = na.omit(ukbb_PAH)
ukbb_PAH$PAH = ifelse(ukbb_PAH$PAH == 0, 0, 1)
print(table(ukbb_PAH$PAH))

write.table(ukbb_PAH,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)