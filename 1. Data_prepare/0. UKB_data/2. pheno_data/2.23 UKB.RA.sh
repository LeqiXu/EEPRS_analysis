...
disease="RA"
RA_icd10 <- c(paste0('M05',c(0:3,8,9)),
...
ukbb_RA = case_summary[,c("eid","RA")]
ukbb_RA = na.omit(ukbb_RA)
ukbb_RA$RA = ifelse(ukbb_RA$RA == 0, 0, 1)
print(table(ukbb_RA$RA))

write.table(ukbb_RA,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)