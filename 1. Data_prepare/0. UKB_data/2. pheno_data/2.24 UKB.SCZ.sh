...
disease="SCZ"
SCZ_icd10 <- c(paste0('F20',c(0:6,8,9)))
SCZ_icd9 <- NULL
SCZ_ops <- NULL
SCZ_s1 <- c(2) # Field 20544
SCZ_s2 <- c(1289) # Field 20002
...
ukbb_SCZ = case_summary[,c("eid","SCZ")]
ukbb_SCZ = na.omit(ukbb_SCZ)
ukbb_SCZ$SCZ = ifelse(ukbb_SCZ$SCZ == 0, 0, 1)
print(table(ukbb_SCZ$SCZ))

write.table(ukbb_SCZ,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)