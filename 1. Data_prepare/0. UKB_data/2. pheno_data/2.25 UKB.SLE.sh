...
disease="SLE"
SLE_icd10 <- c('M320','M321','M328','M329')
SLE_icd9 <- NULL
SLE_ops <- NULL
SLE_s1 <- c(1381)
SLE_s2 <- NULL
...
ukbb_SLE = case_summary[,c("eid","SLE")]
ukbb_SLE = na.omit(ukbb_SLE)
ukbb_SLE$SLE = ifelse(ukbb_SLE$SLE == 0, 0, 1)
print(table(ukbb_SLE$SLE))

write.table(ukbb_SLE,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)