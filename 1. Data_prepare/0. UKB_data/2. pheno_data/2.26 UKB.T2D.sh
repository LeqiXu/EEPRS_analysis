...
ukbb_T2D = case_summary[,c("eid","T2D")]
ukbb_T2D = na.omit(ukbb_T2D)
ukbb_T2D$T2D = ifelse(ukbb_T2D$T2D == 0, 0, 1)
print(table(ukbb_T2D$T2D))

write.table(ukbb_T2D,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)