...
disease="Asthma"
Asthma_icd10 <- c(paste0('J45',c(0,1,8,9)),'J46')
Asthma_icd9 <- NULL
Asthma_ops <- NULL
Asthma_s1 <- c(1111)
Asthma_s2 <- NULL
...
ukbb_Asthma = case_summary[,c("eid","Asthma")]
ukbb_Asthma = na.omit(ukbb_Asthma)
ukbb_Asthma$Asthma = ifelse(ukbb_Asthma$Asthma == 0, 0, 1)
print(table(ukbb_Asthma$Asthma))

write.table(ukbb_Asthma,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)