...
disease="AF"
AF_icd10 <- c(paste0('I48',c(0:4,9)))
AF_icd9 <- c('4273')
AF_ops <- c('K571',paste0('K62',1:4))
AF_s1 <- c(1471)
AF_s2 <- NULL
...
ukbb_AF = case_summary[,c("eid","AF")]
ukbb_AF = na.omit(ukbb_AF)
ukbb_AF$AF = ifelse(ukbb_AF$AF == 0, 0, 1)
table(ukbb_AF$AF)

write.table(ukbb_AF,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)