...
ukbb_ADHD = case_summary[,c("eid","ADHD")]
ukbb_ADHD = na.omit(ukbb_ADHD)
ukbb_ADHD$ADHD = ifelse(ukbb_ADHD$ADHD == 0, 0, 1)
print(table(ukbb_ADHD$ADHD))

write.table(ukbb_ADHD,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)