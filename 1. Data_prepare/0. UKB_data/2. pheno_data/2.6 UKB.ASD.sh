...
ukbb_ASD = case_summary[,c("eid","ASD")]
ukbb_ASD = na.omit(ukbb_ASD)
ukbb_ASD$ASD = ifelse(ukbb_ASD$ASD == 0, 0, 1)
print(table(ukbb_ASD$ASD))

write.table(ukbb_ASD,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)