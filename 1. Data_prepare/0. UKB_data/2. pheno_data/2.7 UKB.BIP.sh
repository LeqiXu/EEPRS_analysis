...
ukbb_BIP = case_summary[,c("eid","BIP")]
ukbb_BIP = na.omit(ukbb_BIP)
ukbb_BIP$BIP = ifelse(ukbb_BIP$BIP == 0, 0, 1)
print(table(ukbb_BIP$BIP))

write.table(ukbb_BIP,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)