...
disease="BrC"
BrC_icd10 <- c(paste0('C50',0:9))
BrC_icd9 <- c(paste0('174',c(0,3,5,8,9)))
BrC_ops <- NULL
BrC_s1 <- c(1002) # field 20001
BrC_s2 <- NULL
...
ukbb_BrC = case_summary[,c("eid","BrC")]
ukbb_BrC = na.omit(ukbb_BrC)
ukbb_BrC$BrC = ifelse(ukbb_BrC$BrC == 0, 0, 1)
print(table(ukbb_BrC$BrC))

write.table(ukbb_BrC,paste0(output_path,'ukbb_',disease,'.tsv'),
	    quote=F,sep='\t',row.names=F)