...
## Prepare data
train_embedding_transformed = fread(paste0("word2vec100_EUR_train_embed_normalize.txt"))
...
write.table(train_pca_embedding, file=paste0("word2vec100_PCA",idx80,"_EUR_train_embed_normalize.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
...
write.table(train_ica_embedding, file=paste0("word2vec100_ICA",idx80,"_EUR_train_embed_normalize.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")