...
## PCA
job_file="/gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/word2vec100_PCA30_corr.txt"
> $job_file  # Empty the job file if it already exists

train_type=train

for i in {1..29}; do
for j in $(seq $((i+1)) 30); do

# Construct the sumstats files for embeddings i and j (comma-separated)
file_i="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_PCA30_${train_type}_EUR_UKB_PC${i}_ldsc.sumstats.gz"
file_j="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_PCA30_${train_type}_EUR_UKB_PC${j}_ldsc.sumstats.gz"
    
out_prefix="/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/embedding_embedding_corr/word2vec100_PCA30_${train_type}_EUR_UKB_PC${i}_${j}_rg"
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/word2vec100_PCA30_corr.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=1-00:00:00 --mail-type=ALL
sbatch dsq-word2vec100_PCA30_corr-$(date +%Y-%m-%d).sh

## ICA
job_file="/gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/word2vec100_ICA30_corr.txt"
> $job_file  # Empty the job file if it already exists
...
# Construct the sumstats files for embeddings i and j (comma-separated)
file_i="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_ICA30_${train_type}_EUR_UKB_V${i}_ldsc.sumstats.gz"
file_j="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_ICA30_${train_type}_EUR_UKB_V${j}_ldsc.sumstats.gz"
    
out_prefix="/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/embedding_embedding_corr/word2vec100_ICA30_${train_type}_EUR_UKB_V${i}_${j}_rg"
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/word2vec100_ICA30_corr.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=1-00:00:00 --mail-type=ALL
sbatch dsq-word2vec100_ICA30_corr-$(date +%Y-%m-%d).sh
...
read_correlation_logs <- function(pattern, prefix) {
  ...
  for (f in files) {
    ...
    # e.g. "word2vec100_PCA30_train_EUR_UKB_PC(\\d+)_(\\d+)_rg\\.log"
...
pca_results <- read_correlation_logs(
  "^word2vec100_PCA30_train_EUR_UKB_PC(\\d+)_(\\d+)_rg\\.log$",
  "PCA"
)
ica_results <- read_correlation_logs(
  "^word2vec100_ICA30_train_EUR_UKB_V(\\d+)_(\\d+)_rg\\.log$",
  "ICA"
)
...
write.table(pca_results, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/embedding_embedding_corr/UKB_word2vec100_pca_train_corr.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")

ica_results$embedding_approach = "GPT_ICA"
ica_results

write.table(ica_results, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/embedding_embedding_corr/UKB_word2vec100_ica_train_corr.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
...
pca_sig_embedding <- paste0("PCA_",c(1,2,7,9,4,3,5,18,11,8,20,15,6,10,16,17,14,13,21,25,26,12,27,24))
ica_sig_embedding <- paste0("ICA_",c(25,19,12,2,15,11,22,27,4,14,24,5,8,1,10,20,16,13,9,6,29,7))
...
pca_plot <- grid::grid.grabExpr(
  pheatmap(
    ...
    number_color    = "black",
    main            = "Genetic Correlation Estimates (Word2Vec_PCA)",
    legend          = TRUE,
    treeheight_row  = 0,
    treeheight_col  = 0
  )
...
ica_plot <- grid::grid.grabExpr(
  pheatmap(
    ...
    number_color    = "black",
    main            = "Genetic Correlation Estimates (Word2Vec_ICA)",
    legend          = TRUE,
    treeheight_row  = 0,
    treeheight_col  = 0
  )
