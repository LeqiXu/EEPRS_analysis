...
job_file="/gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_word2vec100_PCA30_corr.txt"
...
file_j="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_PCA30_${train_type}_EUR_UKB_PC${emd_i}_ldsc.sumstats.gz"
    
out_prefix="/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/${trait}_word2vec100_PCA30_${train_type}_EUR_UKB_PC${emd_i}_rg"
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_word2vec100_PCA30_corr.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=1-00:00:00 --mail-type=ALL
sbatch dsq-trait_word2vec100_PCA30_corr-$(date +%Y-%m-%d).sh
...
get_trait_correlations <- function(trait, directory) {
  ...
  pattern <- paste0("^", trait, "_word2vec100_PCA30_train_EUR_UKB_PC[0-9]+_rg\\.log$")
  ...
  get_embedding_number <- function(filename) {
    sub(paste0("^", trait, "_word2vec100_PCA30_train_EUR_UKB_PC([0-9]+)_rg\\.log$"),
        "\\1",
        basename(filename))
  }
...
write.table(all_results, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/UKB_word2vec100_pca_train_trait_corr.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
...
pheatmap(
  ...
  number_color    = "black",
  main            = "Genetic Correlation Estimates (51 Trait x Word2Vec_PCA)",
  treeheight_row  = 0,
  treeheight_col  = 0
)
...
write.table(sig_table, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/trait_significant_word2vec_pca_embeddings.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
