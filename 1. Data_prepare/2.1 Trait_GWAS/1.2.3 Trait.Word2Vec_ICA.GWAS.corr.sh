...
job_file="/gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_word2vec100_ICA30_corr.txt"
...
file_j="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/word2vec100_ICA30_${train_type}_EUR_UKB_V${emd_i}_ldsc.sumstats.gz"
    
out_prefix="/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/${trait}_word2vec100_ICA30_${train_type}_EUR_UKB_V${emd_i}_rg"
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_word2vec100_ICA30_corr.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=1-00:00:00 --mail-type=ALL
sbatch dsq-trait_word2vec100_ICA30_corr-$(date +%Y-%m-%d).sh
...
write.table(all_results, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/UKB_word2vec100_ica_train_trait_corr.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
...
pheatmap(
  ...
  number_color    = "black",
  main            = "Genetic Correlation Estimates (51 Trait x Word2Vec_ICA)",
  treeheight_row  = 0,
  treeheight_col  = 0
)
...
write.table(sig_table, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/trait_significant_word2vec_ica_embeddings.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
