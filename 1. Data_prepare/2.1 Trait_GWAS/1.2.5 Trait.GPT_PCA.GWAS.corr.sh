...
job_file="/gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_gpticd3072_PCA67_corr.txt"
...
file_j="/gpfs/gibbs/pi/zhao/lx94/EEPRS/data/embedding_data/munge_data/gpticd3072_PCA67_${train_type}_EUR_UKB_PC${emd_i}_ldsc.sumstats.gz"
    
out_prefix="/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/${trait}_gpticd3072_PCA67_${train_type}_EUR_UKB_PC${emd_i}_rg"
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/EEPRS/code/glm/trait_gpticd3072_PCA67_corr.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=1-00:00:00 --mail-type=ALL
sbatch dsq-trait_gpticd3072_PCA67_corr-$(date +%Y-%m-%d).sh
...
get_trait_correlations <- function(trait, directory) {
  ...
  pattern <- paste0("^", trait, "_gpticd3072_PCA67_train_EUR_UKB_PC[0-9]+_rg\\.log$")
  ...
  get_embedding_number <- function(filename) {
    sub(paste0("^", trait, "_gpticd3072_PCA67_train_EUR_UKB_PC([0-9]+)_rg\\.log$"),
        "\\1",
        basename(filename))
  }
...
get_trait_correlations <- function(trait, directory) {
  ...
  pattern <- paste0("^", trait, "_word2vec100_ICA30_train_EUR_UKB_V[0-9]+_rg\\.log$")
  ...
  # A small helper to extract the embedding number from the filename
  get_embedding_number <- function(filename) {
    sub(paste0("^", trait, "_word2vec100_ICA30_train_EUR_UKB_V([0-9]+)_rg\\.log$"),
        "\\1",
        basename(filename))
  }
...
write.table(all_results, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/UKB_gpticd3072_pca_train_trait_corr.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
...
pca_sig_embedding <- c(2,3,4,12,6,1,9,15,14,5,22,21,38,34,10,18,11,20,16,42,40,32,19,24,23,33,26,37,17,7,44,55,56,8,43,54,45,29,48,39,25,35,65,46,51,31,28,67,13,51,30,60)
all_results = all_results[which(all_results$emd_i %in% pca_sig_embedding ),]
...
pheatmap(
  ...
  number_color    = "black",
  main            = "Genetic Correlation Estimates (51 Trait x GPT_PCA)",
  treeheight_row  = 0,
  treeheight_col  = 0
)
...
write.table(sig_table, file=paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/h2_corr/corr/trait_embedding_corr/trait_significant_gpt_pca_embeddings.txt"), row.names=F, col.names=T, quote=F, append=F, sep = "\t")
