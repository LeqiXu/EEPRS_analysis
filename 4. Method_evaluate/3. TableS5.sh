...
MTAG_PRS = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/MTAG_meta_PRS/",trait,"/UKB_test_1over3_",trait,"_MTAG_PRS1.txt"))
if (length(unique(MTAG_PRS$MTAG_PRS)) == 1 && unique(MTAG_PRS$MTAG_PRS) == 0) {
  MTAG_PRS = PRScs[, c("IID","SCORE1_AVG")]
  colnames(MTAG_PRS) = c("IID","MTAG_PRS")
}
MTAG_PRS <- as.data.frame(MTAG_PRS)
...
MTAG_PRS = MTAG_PRS[match(Trait_pheno_id,MTAG_PRS$IID),]
...
test_data = data.table(PRScs = scale(PRScs$SCORE1_AVG),
                       ...
                       MTAG_PRS = scale(MTAG_PRS$MTAG_PRS))
colnames(test_data) = c("PRScs","EEPRS_Word2Vec","EEPRS_Word2Vec_PCA","EEPRS_Word2Vec_ICA","EEPRS_GPT_PCA","EEPRS_GPT_ICA","MTAG_PRS")
...
all_prs_cols = c("PRScs", "EEPRS_Word2Vec","EEPRS_Word2Vec_PCA","EEPRS_Word2Vec_ICA","EEPRS_GPT_PCA","EEPRS_GPT_ICA","MTAG_PRS")
...
test_perf = data.table(fold = f,trait = trait,method = c("PRScs", "MTAG_PRS","MTAG_EEPRS"),performance = NA_real_)
...
if (trait %in% continuous_trait) {
    ...
    prs_fit_test = lm(as.formula(paste("pheno ~", paste(cov_choice, collapse = " + "), "+ MTAG_PRS")), data=test_dt)
    prs_ssr = sum(resid(prs_fit_test)^2)
    test_perf[method=="MTAG_PRS", performance := 1 - prs_ssr/null_ssr_test]

} else {
    prs_fit_test = glm(as.formula(paste("pheno ~", " MTAG_PRS")), data=test_dt, family=binomial(link="logit"))
    prs_probs = predict(prs_fit_test, newdata=test_dt, type="response")
    auc_val = pROC::roc(test_dt$pheno, prs_probs, quiet=TRUE)$auc
    test_perf[method=="MTAG_PRS", performance := auc_val]
}
...
if (trait %in% continuous_trait) {
    ...
    form_str = paste("pheno ~", paste(cov_choice, collapse = " + "), "+", eeprs_best, " + MTAG_PRS")
    fit = lm(as.formula(form_str), data=train_dt)
    test_dt$MTAG_EEPRS = predict(fit, newdata = test_dt)

    eeprs_fit_test = lm(as.formula(paste("pheno ~", paste(cov_choice, collapse = " + "), "+ MTAG_EEPRS")), data=test_dt)
    eeprs_ssr = sum(resid(eeprs_fit_test)^2)
    eeprs_r2 = 1 - eeprs_ssr/null_ssr_test
    test_perf[method=="MTAG_EEPRS", performance := eeprs_r2]
} else {
    form_str = paste("pheno ~", eeprs_best, " + MTAG_PRS")
    fit = glm(form_str, data=train_dt, family=binomial(link="logit"))
    test_dt$MTAG_EEPRS = predict(fit, newdata = test_dt, type = "link")

    eeprs_fit_test = glm(as.formula(paste("pheno ~ MTAG_EEPRS")), data=test_dt, family=binomial(link="logit"))
    eeprs_probs = predict(fit, newdata = test_dt, type = "response")
    eeprs_auc = pROC::roc(test_dt$pheno, eeprs_probs, quiet=TRUE)$auc
    test_perf[method=="MTAG_EEPRS", performance := eeprs_auc]
}
...
write.table(final_cv_results,paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/evaluation/MTAG_EEPRS_optimal_R2_AUC.csv"),quote=F,sep='\t',row.names=F,col.names=T)

wide_table <- pivot_wider(final_cv_results, 
                          names_from = method, 
                          values_from = mean_performance)
write_xlsx(wide_table,paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/table/TableS5.xlsx"))