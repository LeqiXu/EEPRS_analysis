...
process_type <- function(type) {
  ...
  #---------------------------------------
  ordered_labels <- sapply(sig_embedding, function(ii) paste0(title_type, "_V", ii))
...
p <- ggplot(final_df, aes(x = group, y = prs_label, fill = max_logp)) +
  geom_tile() +
  geom_text(aes(label = sig_label), color = "black", size = 3, fontface = "bold") +
  scale_fill_gradientn(
    colors = c("white", "yellow", "red"),
    name = expression(paste("max ", -log[10], "(BH_p)"))
  ) +
  labs(
    title = "Combined Heat Map of Two Embedding Types",
    subtitle = "Colored by max BH_p; star (*) indicates BH_p < 0.05",
    x = "PheWAS Group",
    y = "Embedding PRS"
  ) +
  facet_wrap(~ title_type, scales = "free_y") +   # or scales="free" if you want the groups also separate
