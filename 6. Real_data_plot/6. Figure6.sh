...
## 2. Data
final_cv_results = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/EEPRS/result/evaluation/EEPRS_optimal_R2_AUC.csv"))
comparison_continuous_table <- final_cv_results %>%
  ...
  mutate(
    difference      = EEPRS_optimal - PRScs,
    relative_change = (EEPRS_optimal - PRScs) / PRScs
  ) %>%
...
comparison_binary_table <- final_cv_results %>%
  ...
  mutate(
    difference      = EEPRS_optimal - PRScs,
    relative_change = (EEPRS_optimal - PRScs) / (PRScs - 0.5)
  ) %>%
  arrange(desc(relative_change))
comparison_binary_table = comparison_binary_table[which(comparison_binary_table$PRScs > 0.5),]
comparison_binary_table = comparison_binary_table[which(comparison_binary_table$EEPRS_optimal > 0.5),]
