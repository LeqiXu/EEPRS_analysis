...
comparison_continuous_table <- final_cv_results %>%
  ...
  mutate(
    difference      = MTAG_EEPRS - MTAG_PRS,
    relative_change = (MTAG_EEPRS - MTAG_PRS) / MTAG_PRS
  ) %>%
...
comparison_binary_table <- final_cv_results %>%
  ...
  mutate(
    difference      = MTAG_EEPRS - MTAG_PRS,
    relative_change = (MTAG_EEPRS - MTAG_PRS) / (MTAG_PRS - 0.5)
  ) %>%
  arrange(desc(relative_change))
comparison_binary_table = comparison_binary_table[which(comparison_binary_table$MTAG_PRS > 0.5),]
